...
const handleLogin = async () => {
  ...
  try {
    ...
    localStorage.setItem('token', accessToken); // Temporarily store so apiClient works
    
    // 2. Fetch user profile
    const profileResponse = await apiClient.get('/users/me');
    
    // 3. Update global store
    authStore.setUser(profileResponse.data.data, accessToken);
    
    router.push('/dashboard');
  } catch (err) {
    error.value = err.response?.data?.detail || err.response?.data?.message || 'Login failed. Please check your credentials.';
  } finally {
    loading.value = false;
  }
};
